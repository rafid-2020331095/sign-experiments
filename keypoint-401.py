...
# ── Settings ──────────────────────────────────────────────────────────────────
NUM_WORDS     = 401    # all 401 words (W001–W401)
...
def collect_split(split_name: str, max_word: int = 401):
    """
...
def process_and_save(entries, split_name: str):
    ...
    sequences, labels, signer_ids, video_stems = [], [], [], []
    done_paths = set()
    ...
    for cf in ckpt_files:
        ...
        if 'video_stem' in d.files:
            video_stems.extend(d['video_stem'].tolist())
        else:
            # backward-compatible: derive stem from stored paths
            video_stems.extend([os.path.splitext(os.path.basename(p))[0]
                                 for p in d['paths'].tolist()])
    if done_paths:
    ...
    failed = []
    buf_seqs, buf_labels, buf_sids, buf_paths, buf_stems = [], [], [], [], []
    ...
    for video_path, label, signer_id in tqdm(remaining, desc=f'[{split_name}]'):
        ...
        buf_stems.append(os.path.splitext(os.path.basename(video_path))[0])
    ...
    X  = np.stack(sequences).astype(np.float32)     # (N, 200, 223)
    y  = np.array(labels, dtype=np.int64)            # (N,)
    s  = np.array(signer_ids, dtype=np.int64)        # (N,)  1-based signer IDs
    vs = np.array(video_stems)                       # (N,)  e.g. 'W001S01F_01'
    np.savez_compressed(save_path, X=X, y=y, signer_id=s, video_stem=vs)
